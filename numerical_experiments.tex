...
\caption{Illustrative frozen flows, not a statistical comparison. Top, left to right: initial classification inputs, full-model images at $T=1$, and random-batch images. These $768$ points pool the saved test, training, and calibration splits solely for visualization; they are not a larger independent test sample. Blue and orange identify classes $-1$ and $+1$; each point carries a dark rim so that individual images remain separable where the cloud collapses, and the faint blurred bundles are the straight-line correspondences between a point and its image. The fixed targets $(-1,0)$ and $(0,1)$ are stated here rather than drawn, so that they do not compete with the transported cloud. Bottom left: initial density and the learned full-flow density at $T=1$, drawn as filled Oranges level sets with thin black isolines on one shared scale whose lowest level is a small positive fraction of the common maximum $2/\pi$; the terminal blob is therefore lighter because its mass has spread, not because it was rescaled. Density values come from characteristics on a mapped mesh, without KDE or renormalization. Bottom middle/right: the same $1024$ independent initial samples (blue) together with their full/random terminal images (orange). These are equal-weight samples, not quadrature nodes. Each row uses common axis limits and aspect ratio. The previous realization is retained: uniform sampling, $h=2^{-8}$, $r=8$ and schedule seed $20260908$ for classification, $r=16$ and seed $20260909$ for transport, and transport sampling seed $20260910$. One schedule is shared by every point in each flow. No realization was selected by appearance.}
\label{fig:rbm_node}
\end{figure}

\Cref{fig:classification_trajectories} shows the same two classification flows as complete paths rather than only their endpoints.

\begin{figure}[htbp]
\centering
\includegraphics[width=.49\linewidth]{Figures/numerics/classification_trajectory_full.pdf}\hfill
\includegraphics[width=.49\linewidth]{Figures/numerics/classification_trajectory_random.pdf}
\caption{Full-model (left) and random-batch (right) classification trajectories at the smallest tested switching scale $h=2^{-11}$, with $\Delta t=h/8$ and schedule seed $20260908$. Every plotted state is a genuine RK4 state at a switching boundary; no straight-line interpolation is used. Blue and orange again identify classes $-1$ and $+1$; the paths are drawn as blurred low-opacity bundles so that their density is legible, and the terminal points keep a dark rim on top. Both panels share the axis limits, which are set from the paths alone. The targets are not drawn.}
\label{fig:classification_trajectories}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{Figures/numerics/transport_coupling.pdf}\\[1ex]
\includegraphics[width=\linewidth]{Figures/numerics/transport_density.pdf}
\caption{Transport at the frozen control. Top (\texttt{transport\_coupling.pdf}): squared coupling \eqref{eq:transport_sq_error} and mean coupling upper bound \eqref{eq:transport_coup_error}, using $40$ schedules and $1024$ quadrature points; the latter is not $W_2$ itself. Bottom (\texttt{transport\_density.pdf}), left: pointwise density MSE from $40$ schedules at $\bfx^{(1)},\ldots,\bfx^{(5)}$, the full-flow images of $(-1,-1)$, $(-0.6,-1)$, $(-1,-0.4)$, $(-1.8,-1)$, $(-1,-1.9)$; each curve is labeled by the initial radius $\|y-c\|$ and they use blue, orange, green, red, and purple in this order. Bottom right: global $L^1$ error from $12$ schedules and $2704$ quadrature points using \eqref{eq:l1_change_of_variables}. Bands are approximate $95\%$ schedule intervals. The dashed lines are unlabeled least-squares log--log trend guides and carry no theoretical claim; the dotted line marks the smallest measured refinement floor. Spatial refinement on three paired schedules per $h$ gives maximum production-to-finer changes ranging from $1.0\times10^{-4}$ to $7.7\times10^{-4}$; these depend on $h$ and are diagnostics, not a common error floor or certified bound.}
\label{fig:transport_summary}
\label{fig:transport_coupling}
\label{fig:transport_density}
\end{figure}

\Cref{fig:transport_density_evolution} shows the transported measure itself, rather than an error against $h$.

\begin{figure}[htbp]
\centering
\includegraphics[width=.78\linewidth]{Figures/numerics/transport_density_full_evolution.pdf}\\[1ex]
\includegraphics[width=.78\linewidth]{Figures/numerics/transport_density_random_evolution.pdf}
\caption{Transported density under the full flow (top) and one random-batch realization (bottom), at $h=2^{-8}$, $r=16$ and schedule seed $20260909$. The leftmost field in each panel is the analytic initial density $\rho_{\mathrm B}$ of \cref{subsec:num_transport}, not an estimate; the four later fields are Gaussian kernel density estimates, with a Silverman-type bandwidth, of $20000$ trajectories of the same initial sample simulated to $t=0.25,0.5,0.75,1$. All five fields, and both panels, share one set of filled Oranges levels whose lowest level is a small positive fraction of the global maximum, so the progressive lightening is the physical spreading of mass and not a per-time rescaling. Bandwidths and the sampling seed are recorded in \texttt{transport\_density\_evolution\_config.json}. This is one realization for illustration; it is not a statistical comparison of the two flows.}
\label{fig:transport_density_evolution}
\end{figure}
