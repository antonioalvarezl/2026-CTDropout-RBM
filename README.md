...
outputs/        Generated data, checkpoints, and figures (not tracked by Git)
```
...
Generated files are written to `outputs/`, which is intentionally excluded
from Git. Experiment calculations and plotting are separate, so figures can be
regenerated without repeating training or integration.
